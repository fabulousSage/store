...
import { useExchangeRateGBPToNGN } from "../../hooks/useExchangeRateGBPToNGN";
...
const CartBox = () => {
  ...
  const irPrice = useExchangeRateGBPToNGN(cartTotalAmount);
          ...
          <div className="flex items-center sticky bottom-0 left-0 right-0 bg-palette-card font-normal py-3 px-4">
            <div className="flex flex-col flex-grow ltr:mr-2 rtl:ml-2">
              ...
              <p className="self-end text-sm font-bold">
                {locale === "en"
                  ? `£ ${ngnCurrencyFormat(cartTotalAmount)}`
                  : `تومان ${irPrice}`}
              </p>
            </div>
            {!userInfo ? (
